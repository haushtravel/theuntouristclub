(function () {
  ...
  function updateSightSlider() {
    ...
    const gap = parseFloat(window.getComputedStyle(sightsTrack).columnGap || window.getComputedStyle(sightsTrack).gap || "0");
    const shift = -(cardWidth + gap) * activeSight;
    root.style.setProperty("--sights-shift", `${shift.toFixed(2)}px`);
  ...
  function setupSightSlider() {
    if (!sightsTrack || originalSightCards.length === 0) return;
